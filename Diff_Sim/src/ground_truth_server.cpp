...
public:
  DenseGroundTruthGenerator(std::vector<Gaussian2D> gaussians,
                            ...
                            double lipschitz_constant, int random_seed)
      : area_min_x_(area_min_x), area_max_x_(area_max_x),
        area_min_y_(area_min_y), area_max_y_(area_max_y),
        lipschitz_constant_(lipschitz_constant), random_seed_(random_seed),
  ...
  double sample(double x, double y) const {
    ...
    for (const auto &gaussian : gaussians_) {
      ...
      double exponent =
          -(dx_rot * dx_rot / (2.0 * var_x) + dy_rot * dy_rot / (2.0 * var_y));
...
public:
  ...
  void generateGaussians(int num_gaussians) {
    ...
    double base_std_dev =
        area_diagonal / (lipschitz_constant_ * std::sqrt(num_gaussians));

    std::uniform_real_distribution<> dis_std(base_std_dev * 0.5,
                                             base_std_dev * 1.5);
    std::uniform_real_distribution<> dis_amp(0.5, 2.0);
    std::uniform_real_distribution<> dis_rotation(
        0.0, 2.0 * M_PI); // Random rotation [0, 2π]
...
public:
  GroundTruthServerNode() : Node("ground_truth_server") {
    ...
    this->declare_parameter<double>("grid_resolution",
                                    0.05); // Cell size in meters
    this->declare_parameter<double>("outside_value", 1.0);
    this->declare_parameter<double>("max_border_slope", 2.0);
    ...
    double lipschitz_constant =
        this->get_parameter("lipschitz_constant").as_double();
    int random_seed = this->get_parameter("random_seed").as_int();
    outside_value_ = this->get_parameter("outside_value").as_double();
    max_border_slope_ = this->get_parameter("max_border_slope").as_double();
      ...
      if (!map_path.is_absolute()) {
        try {
          ...
          std::filesystem::path full_path =
              std::filesystem::path(package_share_dir) / "maps" / map_path;
          ...
          RCLCPP_INFO(this->get_logger(), "Resolved relative map path to: %s",
                      map_file.c_str());
        } catch (const std::exception &e) {
          RCLCPP_WARN(
              this->get_logger(),
              "Failed to resolve package path: %s. Using original path: %s",
        ...
        if (abs_path.extension().empty()) {
          ...
          RCLCPP_INFO(this->get_logger(), "Added .csv extension to path: %s",
                      map_file.c_str());
        }
      }
    }
    ...
    if (if_randomize) {
      generator_ = std::make_unique<DenseGroundTruthGenerator>(
          area_min_x, area_max_x, area_min_y, area_max_y, num_gaussians,
          lipschitz_constant, random_seed);
      setReportedNumGaussians(static_cast<size_t>(num_gaussians));
      RCLCPP_INFO(this->get_logger(), "Randomized terrain with %d gaussians",
                  num_gaussians);
    } else {
      std::vector<Gaussian2D> gs;
      std::string err;
      bool success = DenseGroundTruthGenerator::loadGaussiansFromCSV(
          map_file, area_min_x, area_max_x, area_min_y, area_max_y, gs, err);
      if (!success || gs.empty()) {
        RCLCPP_ERROR(this->get_logger(),
                     "Failed to load gaussians from '%s': %s", map_file.c_str(),
                     err.c_str());
        RCLCPP_ERROR(this->get_logger(),
                     "Falling back to random terrain generation");
        generator_ = std::make_unique<DenseGroundTruthGenerator>(
            area_min_x, area_max_x, area_min_y, area_max_y, num_gaussians,
            lipschitz_constant, random_seed);
        setReportedNumGaussians(static_cast<size_t>(num_gaussians));
    ...
    // Create service
    service_ =
        this->create_service<safe_scout_simulator::srv::SampleGroundTruth>(
            "sample_ground_truth",
    ...
    grid_publisher_ = this->create_publisher<foxglove_msgs::msg::Grid>(
        "ground_truth_map", 10);

    // Generate grid at initialization based on resolution
    RCLCPP_INFO(this->get_logger(),
                "Generating ground truth grid with resolution %.3f m...",
                grid_resolution_);
    generateGridMap();
    RCLCPP_INFO(this->get_logger(),
                "Grid map generated: %d x %d cells (%zu total)", grid_width_,
                grid_height_, grid_values_.size());
    ...
    RCLCPP_INFO(this->get_logger(), "Number of Gaussians: %zu",
                generatorSize());
    RCLCPP_INFO(this->get_logger(), "Lipschitz constant: %.2f",
                lipschitz_constant);
    if (random_seed >= 0) {
      RCLCPP_INFO(this->get_logger(), "Random seed: %d (deterministic)",
                  random_seed);
    } else {
      RCLCPP_INFO(this->get_logger(),
                  "Random seed: random_device (non-deterministic)");
    }
  }

private:
  size_t generatorSize() const {
    // We can't access internal vector size directly; re-sample a few points
    // isn't helpful. Instead, rely on sampling behavior; but for logging, we
    // can approximate by probing. Simplify: maintain a shadow count by checking
    // sampled values is not reliable; here we just return 0 when unknown. For
    // better logging, store size when constructing. To achieve that, store a
    // field.
  ...
  void handleSampleRequest(
      const std::shared_ptr<
          safe_scout_simulator::srv::SampleGroundTruth::Request>
          request,
      std::shared_ptr<safe_scout_simulator::srv::SampleGroundTruth::Response>
          response) {

    response->value = sampleWithBoundary(request->x, request->y);

    RCLCPP_DEBUG(this->get_logger(), "Sampled (%.2f, %.2f) = %.4f", request->x,
                 request->y, response->value);
  }
  ...
  double sampleWithBoundary(double x, double y) const {
    ...
    if (x >= area_min_x_ && x <= area_max_x_ && y >= area_min_y_ &&
        y <= area_max_y_) {
    ...
    double max_delta = max_border_slope_ * dist_to_edge;
    double min_val = outside_value_ - max_delta;
    double max_val = outside_value_ + max_delta;
  ...
  void generateGridMap() {
    ...
    if (value_range < 1e-9)
      value_range = 1.0; // Avoid division by zero
  ...
  void publishGrid() {
    ...
    grid_msg->fields = {field_value, field_red, field_green, field_blue,
                        field_alpha};
    ...
    for (double normalized_val : grid_values_) {
      ...
      uint8_t value =
          static_cast<uint8_t>(std::clamp(normalized_val * 255.0, 0.0, 255.0));
  ...
  rclcpp::Service<safe_scout_simulator::srv::SampleGroundTruth>::SharedPtr
      service_;
  ...
  double outside_value_ = 1.0;
  double max_border_slope_ = 2.0;
};
