...
int main(int argc, char* argv[]) {
    ...
    ChVector3d Spirit_position = ChVector3d(5, 5, .19);
    ...
    spirit0->LockTrussEndPosition(0, ChVector3d(5-0, 5+.5, .25));
    ...
    while (rclcpp::ok() && vis_irr->Run()) {
        ...
        //Slowly increment shoulder setpoint from 0 to a final setpoint of -.4.
        double setpoint_shoulder1 = -(sim_time)/.25;
        if(setpoint_shoulder1 < -3.14+.4){
            setpoint_shoulder1 = -3.14+.4;
        }
        if(sim_time > .7){
            setpoint_shoulder1 = -3.14+.4;
        }
        ...
        spirit0->SetShoulderDesiredPosition(2,setpoint_shoulder1);
        spirit0->SetShoulderDesiredPosition(3,setpoint_shoulder1);
        ...
        if(g_advance_simulation.load() || sim_time < 4){
            loop_index++;
            previous_spirit0_position = spirit0->m_body->GetPos();
            previous_spirit0_orientation = spirit0->m_body->GetRot();
        ...
        if(sim_time > 4 && g_advance_simulation.load()){
            double setpoint_truss = init_truss_length + (sim_time - 4) * 0.1; // Increment truss length
            if(setpoint_truss > 1){
                setpoint_truss = 1;
            }
            spirit0->m_trusses[0].SetArmDesiredLength(setpoint_truss);
            csv << sim_time << spirit0->m_body->GetPos() << spirit0->m_body->GetRot() << spirit0->m_leg_bodies[0][3]->GetPos()[0] << spirit0->m_leg_bodies[0][3]->GetPos()[1] << spirit0->m_leg_bodies[0][3]->GetPos()[2] << spirit0->m_leg_bodies[1][3]->GetPos()[0] << spirit0->m_leg_bodies[1][3]->GetPos()[1] << spirit0->m_leg_bodies[1][3]->GetPos()[2] << spirit0->m_leg_bodies[2][3]->GetPos()[0] << spirit0->m_leg_bodies[2][3]->GetPos()[1] << spirit0->m_leg_bodies[2][3]->GetPos()[2] << spirit0->m_leg_bodies[3][3]->GetPos()[0] << spirit0->m_leg_bodies[3][3]->GetPos()[1] << spirit0->m_leg_bodies[3][3]->GetPos()[2] << spirit0->m_trusses[0].GetLength() << setpoint_truss << spirit0->m_trusses[0].motor->GetMotorForce() << std::endl;
        }
    ...
    csv.WriteToFile(out_dir + "/spirit_force_calibration.dat");
