#include "sim_tools/ROSRHex.h"
  ...
  ROSRHex::ROSRHex(const std::string &node_name) : Node(node_name){ 
    ...
    set_joint_pos_sub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
      ...
      std::bind(&ROSRHex::set_joint_pos_callback, this, std::placeholders::_1));

    set_joint_PID_sub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
      node_name+"/set_joint_PID", 10, 
      std::bind(&ROSRHex::set_joint_PID_callback, this, std::placeholders::_1));

    set_truss_lengths_sub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
      node_name+"/set_truss_lengths", 10, 
      std::bind(&ROSRHex::set_truss_lengths_callback, this, std::placeholders::_1));
  }

  // Method to publish pose value
  void ROSRHex::publish_pose(ChVector3d position, ChQuaterniond quat)
  {
  ...
  void ROSRHex::publish_joint_angles(std::vector<double>& angles){
...
void ROSRHex::publish_toe_pos(std::vector<ChVector3d>& positions){
    auto message = std_msgs::msg::Float32MultiArray();
...
void ROSRHex::publish_truss_lengths(std::vector<double>& lengths){
    auto message = std_msgs::msg::Float32MultiArray();
...
void ROSRHex::set_joint_pos_callback(const std::shared_ptr<const std_msgs::msg::Float32MultiArray> msg){set_joint_pos_msg = *msg; }
void ROSRHex::set_joint_PID_callback(const std::shared_ptr<const std_msgs::msg::Float32MultiArray> msg){set_joint_PID_msg = *msg; }
void ROSRHex::set_truss_lengths_callback(const std::shared_ptr<const std_msgs::msg::Float32MultiArray> msg){set_truss_lengths_msg = *msg; }