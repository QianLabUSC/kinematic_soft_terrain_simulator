...
int main(int argc, char* argv[]) {
    ...
    while (unstable && rclcpp::ok() && vis_irr->Run()) {
        ...
        if(sim_time >1 && run_once){
            run_once = false;

            auto link0 = std::make_shared<ChLinkLockRevolute>();
            link0->Initialize(world, spirit0->m_leg_bodies[0][3], ChFrame<>(spirit0->m_leg_bodies[0][3]->GetPos(), QUNIT));
            sys.AddLink(link0);

            auto link1 = std::make_shared<ChLinkLockRevolute>();
            link1->Initialize(world, spirit0->m_leg_bodies[1][3], ChFrame<>(spirit0->m_leg_bodies[1][3]->GetPos(), QUNIT));
            sys.AddLink(link1);

            auto link2 = std::make_shared<ChLinkLockRevolute>();
            link2->Initialize(world, spirit0->m_leg_bodies[2][3], ChFrame<>(spirit0->m_leg_bodies[2][3]->GetPos(), QUNIT));
            sys.AddLink(link2);

            auto link3 = std::make_shared<ChLinkLockRevolute>();
            link3->Initialize(world, spirit0->m_leg_bodies[3][3], ChFrame<>(spirit0->m_leg_bodies[3][3]->GetPos(), QUNIT));
            sys.AddLink(link3);
    ...
    csv.WriteToFile(out_dir + "/spirit_height_nominal.dat");
