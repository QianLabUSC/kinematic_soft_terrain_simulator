...
float position_epsilon = 0.00009; //.00015
...
int main(int argc, char* argv[]) {
    ...
    mterrain.SetSoilParameters(0.2e8,  // Bekker Kphi
                                   ...
                                   15000,      // Mohr cohesive limit (Pa) /15000
    ...
    float spirit2_init_truss_length = spirit2->m_trusses[0].GetLength();
    ...
    while (rclcpp::ok() && vis_irr->Run()) {
        ...
        if(g_advance_simulation.load()){
            ...
            //spirit0TrussController.adjustSetpointWithVelocity(step_size);
            spirit1TrussController.adjustSetpointWithVelocity(step_size);
            //spirit2TrussController.adjustSetpointWithVelocity(step_size);
            spirit0->m_trusses[0].SetArmDesiredLength(spirit0TrussController.getSetpoint());
            spirit1->m_trusses[0].SetArmDesiredLength(spirit1TrussController.getSetpoint());
            spirit2->m_trusses[0].SetArmDesiredLength(spirit2TrussController.getSetpoint());
            ...
            << spirit0->m_trusses[0].GetLength() << spirit0TrussController.getSetpoint() << spirit0->m_trusses[0].motor->GetMotorForce()
            << spirit1->m_trusses[0].GetLength() << spirit1TrussController.getSetpoint() << spirit1->m_trusses[0].motor->GetMotorForce() 
            << spirit2->m_trusses[0].GetLength() << spirit2TrussController.getSetpoint() << spirit2->m_trusses[0].motor->GetMotorForce() 
            << std::endl;

            if(loop_index > 100 && spirit1TrussController.getMaxLimit() == spirit1TrussController.getSetpoint() && position_sma.getAverage() < position_epsilon && orientation_sma.getAverage() < orientation_epsilon){
                loop_index = 0;
                spirit0TrussController.setLimits(0.0, spirit0TrussController.getMaxLimit()+truss_length_increment);
                spirit1TrussController.setLimits(0.0, spirit1TrussController.getMaxLimit()+truss_length_increment);
                spirit2TrussController.setLimits(0.0, spirit2TrussController.getMaxLimit()+truss_length_increment);
                if(truss_length_increment_counter >= 10){
    ...
    csv.WriteToFile(out_dir + "/3spirit_3truss_model_verification.dat");
